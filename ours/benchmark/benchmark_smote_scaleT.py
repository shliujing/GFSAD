...
def benchmark(ts, df, rf, scale):
    ...
    for i in range(1, len(timestamp)):
        ...
        x = np.array(f.get_features(ts, timestamp[i])).reshape(1, -1)
        x = scale.transform(x)
        y.append(rf.predict(x)[0])
    return y
...
log = []
columns = ['value', 'mean_now', 'std_now', 'minus_mean_now_past', 'minus_mean_now_yes', 'minus_std_now_past',
           'minus_std_now_yes', 'r_mean_now_past', 'r_mean_now_yes', '1', '2', '3', '4', '5', '6', '7', '8', 'ewma',
           'pre_minus', 'pre_rate', 'yes_minus', 'yes_rate', 'label']
for num in numbers:
    print(num)
    ...
    rf, scale = trainScaleSmoteT(df, ts, columns, mark)
    print('training done')
    ...
    log_tmp.to_csv(num + 'scale_smoteT.csv')
